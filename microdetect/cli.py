...
def setup_convert_parser(subparsers):
    ...
    parser.add_argument('--delete_original', action='store_true', help='Excluir arquivos originais após conversão')
...
def setup_visualize_parser(subparsers):
    ...
    parser.add_argument('--label_dir', help='Diretório com anotações (se diferente do diretório de imagens)')
    parser.add_argument('--output_dir', help='Diretório para salvar imagens anotadas')
    parser.add_argument('--filter_classes', help='Lista separada por vírgulas de IDs de classe para exibir (ex: "0,1")')
...
def setup_train_parser(subparsers):
    ...
    parser.add_argument('--data_yaml', help='Caminho para arquivo data.yaml (se não especificado, será criado)')
    ...
    parser.add_argument('--find_hyperparams', action='store_true', help='Buscar melhores hiperparâmetros')
...
def setup_init_parser(subparsers):
    ...
    parser.add_argument('--directory', '-d', default='.', help='Diretório para inicializar (padrão: diretório atual)')
...
def handle_convert(args):
    ...
    if args.format == 'tiff-to-png':
        ...
        success, errors, error_messages = converter.batch_convert(
            args.input_dir, args.output_dir, source_format, target_format,
            args.use_opencv, args.delete_original
        )

    logger.info(f"Conversão concluída: {success} sucessos, {errors} erros")
    if error_messages:
        logger.warning(f"Erros durante a conversão: {', '.join(error_messages[:5])}" +
...
def handle_dataset(args):
    ...
    logger.info(f"Preparação de dataset concluída:"
