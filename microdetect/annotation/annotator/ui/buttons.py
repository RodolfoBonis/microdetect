...
class ButtonManager:
    ...
    def create_main_buttons(self):
        ...
        self.buttons['reset'] = tk.Button(
            button_frame,
            text="Reiniciar (R)",
            command=lambda: self._call_callback('reset')
        )
        self.buttons['reset'].pack(side=tk.LEFT, padx=5)

        self.buttons['undo'] = tk.Button(
            button_frame,
            text="Desfazer (Z)",
            command=lambda: self._call_callback('undo')
        )
        self.buttons['undo'].pack(side=tk.LEFT, padx=5)

        self.buttons['save'] = tk.Button(
            button_frame,
            text="Salvar (S)",
            command=lambda: self._call_callback('save')
        )
        ...
        self.buttons['edit_mode'] = tk.Button(
            button_frame,
            text="Modo Edição (E)",
            command=lambda: self._call_callback('toggle_edit_mode')
        )
        self.buttons['edit_mode'].pack(side=tk.LEFT, padx=5)

        self.buttons['pan_mode'] = tk.Button(
            button_frame,
            text="Modo Navegação (P)",
            command=lambda: self._call_callback('toggle_pan_mode')
        )
        ...
        self.buttons['reset_zoom'] = tk.Button(
            button_frame,
            text="Reset Zoom (W)",
            command=lambda: self._call_callback('reset_zoom')
        )
        ...
        self.buttons['cycle_class'] = tk.Button(
            button_frame2,
            text="Alternar Classe (C)",
            command=lambda: self._call_callback('cycle_classes')
        )
        ...
        self.buttons['exit'] = tk.Button(
            button_frame2,
            text="Sair (Q)",
            command=lambda: self._call_callback('on_closing')
        )
        self.buttons['exit'].pack(side=tk.LEFT, padx=5)

        self.buttons['save_exit'] = tk.Button(
            button_frame2,
            text="Salvar e Sair (X)",
            command=lambda: self._call_callback('save_and_exit')
        )
        self.buttons['save_exit'].pack(side=tk.LEFT, padx=5)

        self.buttons['delete'] = tk.Button(
            button_frame2,
            text="Excluir Seleção (Del)",
            command=lambda: self._call_callback('delete_selected')
        )
        ...
        self.buttons['stats'] = tk.Button(
            button_frame2,
            text="Estatísticas (S)",
            command=lambda: self._call_callback('show_statistics')
        )
        ...
        self.buttons['search'] = tk.Button(
            ...
            command=lambda: self._call_callback('show_search_dialog')
        )
        ...
        self.buttons['export_import'] = tk.Button(
            ...
            command=lambda: self._call_callback('show_export_import')
        )
