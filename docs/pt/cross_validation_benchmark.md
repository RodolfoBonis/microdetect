...
Para comparar diferentes tamanhos ou configurações de modelo:
...
Isso gera:
- Métricas comparativas de desempenho (mAP50, recall, precisão)
- Medições de velocidade para cada modelo
- Visualizações mostrando o trade-off entre precisão e velocidade
- Comparação de tamanho (parâmetros do modelo e tamanho do arquivo)

### Comparação Avançada

Para comparação mais interativa:

```bash
microdetect compare_models --model_paths model1.pt,model2.pt,model3.pt --data_yaml dataset/data.yaml --dashboard
```

Isso inicia um dashboard interativo mostrando:
- Comparação lado a lado de métricas
- Visualização de trade-off (precisão vs. velocidade)
- Comparação de tamanho do modelo
